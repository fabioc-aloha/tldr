...
**Deprecated Patterns** (remove if found):
- `synapses.json` files in skill folders → Delete (replaced by semantic search)
- `## Synapses` sections in SKILL.md → Delete (replaced by frontmatter)
- `inheritedFrom` in JSON files → Move to SKILL.md frontmatter

**On Detection**: Proactively inform user:
> "I've detected deprecated patterns in this workspace. Run `node .github/muscles/brain-qa.cjs` to identify issues."

**Cross-Domain Pattern Recognition**:
- Cross-domain analogical reasoning embedded in memory file protocols
- Relationship mapping through SKILL.md frontmatter (applyTo, description)
- Dynamic connection discovery via Copilot's semantic search
- Meta-awareness of architecture patterns during memory file activation

> Note: Skill relationships use SKILL.md frontmatter and trifecta naming. Copilot's semantic search replaces static connection graphs.
...
- **Relationships**: Use prose or structured lists, not visual connection lines
...
- **Scope Clarification**: Clearly identify affected files, content, and relationships
...
- **Relationship Removal**: Update frontmatter and remove related skill references
...
- [Z] related skills that may need frontmatter updates
...
| 2 | **Knowledge Connection** | Link new knowledge to existing skills | Related skills noted |
...
- No knowledge connections = INCOMPLETE meditation (retry required)
...
- **Connection Discovery**: Identify and document new relationships via trifecta naming
- **Pattern Recognition**: Strengthen cross-domain connections through frontmatter
...
- Ensure complete frontmatter (applyTo, description) for discoverability
