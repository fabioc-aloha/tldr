#!/usr/bin/env node
/**
 * Skill Weight Update -- PostToolUse Hook
 ...
 *
 * Strategy: Buffers activations in skill-activation-buffer.json to avoid
 * write contention on connections.json. Flushes to actual skill files only
...
/** Activations needed before flushing a strength bump to connections.json */
...
if (skillsToFlush.length > 0) {
  ...
  for (const skill of skillsToFlush) {
    // Check connections.json first, fall back to legacy synapses.json
    let connPath = path.join(skillsDir, skill, "connections.json");
    if (!fs.existsSync(connPath)) {
      connPath = path.join(skillsDir, skill, "synapses.json");
    }
    ...
    try {
      ...
      if (!Array.isArray(connData.connections)) continue;
      ...
      if (modified) {
        connData.lastActivated = new Date().toISOString().split("T")[0];
        fs.writeFileSync(
          connPath,
          JSON.stringify(connData, null, 2) + "\n",
          "utf8",
        );
      }
    } catch {
      // Silent -- never fail the tool call for connection bookkeeping
    }
