...
5. **Use Workload Identity Federation for production auth** — for local dev, use a client secret on the Blueprint
...
|-------|-------|----------|
| 403 on API call | Azure CLI token used | Use dedicated app registration with client_credentials |
| 400: Blueprint Principal does not exist | Step 2 skipped | Create BlueprintPrincipal explicitly |
| 403 after admin consent | Propagation delay | Wait 30–120s, retry with backoff |
| Sponsors rejected | Non-User sponsor | Only User objects accepted as sponsors |
| 404 on admin consent | SP not replicated | Wait 10–40s, retry |
